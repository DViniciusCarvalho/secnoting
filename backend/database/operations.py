...
def create_tables(user_id):
    ...
    for table in tables:
        query = text(f"CREATE TABLE {table}_{user_id} ( id INT AUTO_INCREMENT NOT NULL, title VARCHAR(200) NOT NULL, content TEXT NOT NULL, timestamp BIGINT NOT NULL, PRIMARY KEY (id) )")
...
def create_new_annotation(user_id, title, content, timestamp):
    query = text(f"INSERT INTO folders_{user_id} (title, content, timestamp) VALUES (:title, :content, :timestamp)")
    values = { "title": title, "content": content, "timestamp": int(timestamp) }
    Connection.execute_query_with_value(query, values)
...
def add_to_deleteds(user_id, title, content, timestamp):
    ...
    values = { "title": title, "content": content, "timestamp": timestamp }
    result = Connection.execute_query_with_value(query, values)
    if result:
        return True
    return False
...
def update_annotation(table, user_id, title, content, timestamp, annotation_id):
    ...
    values = {"title": title, "content": content, "timestamp": timestamp, "annot_id": annotation_id}    
    result = Connection.execute_query_with_value(query, values)
    if result:
        return True
    return False
